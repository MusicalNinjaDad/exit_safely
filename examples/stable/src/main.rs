...
fn main() -> Exit<()> {
    ...
    let Some(filename) = args.nth(1) else {
        return Exit::InvocationError("REALLY Not enough args".to_string());
    };

    let Some(pass_or_fail) = args.next() else {
        return Exit::InvocationError("Not enough args".to_string());
    };

    let contents = match std::fs::read_to_string(&filename) {
        Ok(contents) => contents,
        Err(e) => return Exit::from(e),
    };
